<form action="/karyawan/{{ Crypt::encrypt($karyawan->nik) }}/update" method="POST" id="frmEditkaryawan"
    ...
    <div class="row">
        <div class="col-12">
            <div class="input-icon mb-3">
                ...
                <input type="text" value="{{ $karyawan->nik }}" id="nik" class="form-control" placeholder="Kode Guru / Karyawan"
                    name="nik_baru">
            </div>
        </div>
    </div>
    ...
    <div class="row mt-2">
        <div class="col-12">
            <select name="kode_tempat" id="kode_tempat" class="form-select">
                <option value="">Tempat</option>
                @foreach ($tempat as $d)
                    <option {{ $karyawan->kode_tempat == $d->kode_tempat ? 'selected' : '' }}
                        value="{{ $d->kode_tempat }}">{{ strtoupper($d->nama_tempat) }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<script>
    $("#frmEditkaryawan").submit(function() {
        ...
        var kode_tempat = $("#frmEditkaryawan").find("#kode_tempat").val();


        if (nik == "") {
            // alert('Kode Guru / Karyawan Harus Diisi');
            Swal.fire({
                title: 'Warning!',
                text: 'Kode Guru / Karyawan Harus Diisi Diisi !',
                icon: 'warning',
