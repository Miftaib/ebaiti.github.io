...
class PresensiController extends Controller
{
    ...
    public function create($kode_jam_kerja)
    {
        ...
        $kode_tempat = Auth::guard('karyawan')->user()->kode_tempat;
        $lok_kantor = DB::table('tempat')->where('kode_tempat', $kode_tempat)->first();
    ...
    public function store(Request $request)
    {
        ...
        $jam = date("H:i:s");
        $lok_kantor = DB::table('tempat')->where('kode_tempat', $kode_tempat)->first();
        $lok = explode(",", $lok_kantor->lokasi_tempat);
        ...
        if ($status_location == 1 && $radius > $lok_kantor->radius_tempat) {
    ...
    public function monitoring()
    {
        ...
        return view('presensi.monitoring', compact('tempat', 'departemen'));
    }

    public function getpresensi(Request $request)
    {
        $kode_dept = Auth::guard('user')->user()->kode_dept;
        $kode_tempat = Auth::guard('user')->user()->kode_tempat;
        ...
        $query->selectRaw(
            'karyawan.nik, nama_lengkap, karyawan.kode_dept, karyawan.kode_tempat,
        ...
        if (!empty($request->kode_tempat)) {
            $query->where('karyawan.kode_tempat', $request->kode_tempat);
        }
        ...
        if ($user->hasRole('admin departemen')) {
            $query->where('karyawan.kode_tempat', $kode_tempat);
        ...
        //         ->where('karyawan.kode_tempat', $kode_tempat)
        //         ->get();
    ...
    public function rekap()
    {
        ...
        $tempat = DB::table('tempat')->orderBy('kode_tempat')->get();
        return view('presensi.rekap', compact('namabulan', 'departemen', 'tempat'));
    }
    ...
    public function cetakrekap(Request $request)
    {
        ...
        if (!empty($kode_tempat)) {
            $query->where('kode_tempat', $kode_tempat);
        }
    ...
    public function izinsakit(Request $request)
    {
        ...
        $tempat = DB::table('tempat')->orderBy('kode_tempat')->get();
        $departemen = DB::table('departemen')->orderBy('kode_dept')->get();
        return view('presensi.izinsakit', compact('izinsakit', 'tempat', 'departemen'));
    }
