...
class DepartemenController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($cek > 0) {
            return Redirect::back()->with(['warning' => 'Sayang Sekali! Data dengan Kode Jabatan ' . $kode_dept . ' Sudah Ada']);
        }
        $simpan = DB::table('departemen')->insert($data);
        if ($simpan) {
            return Redirect::back()->with(['success' => 'Yeay! Data Berhasil Disimpan']);
        } else {
            return Redirect::back()->with(['warning' => 'OhTidak! Data Gagal Disimpan']);
        }
    }
